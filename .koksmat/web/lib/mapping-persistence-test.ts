import { saveMappingsToYaml, loadMappingsFromYaml } from './mapping-persistence'
...
describe('Mapping Persistence', () => {
  ...
  test('saveMappingsToYaml should generate valid YAML with comments', () => {
    ...
    expect(yamlString).toContain('__comment: Mapping from client to server for create operations')
    expect(yamlString).toContain('__comment: Mapping from client to server for update operations')
    expect(yamlString).toContain("fullName: item.first_name + ' ' + item.last_name")
  ...
  test('Round trip should preserve mappings', () => {
    ...
    expect(newYamlString).toContain('__comment: Mapping from client to server for create operations')
    expect(newYamlString).toContain('__comment: Mapping from client to server for update operations')
  })
})