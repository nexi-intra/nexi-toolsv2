import { EntityType, EntityTypeMap, Message, BackendServiceInterface } from './types'
...
export class MessageHandler<T extends EntityType> {
  ...
  private async handleCreate(payload: Omit<EntityTypeMap[T], 'id' | 'createdDate' | 'createdBy' | 'updatedDate' | 'updatedBy'>): Promise<EntityTypeMap[T]> {
    return this.backendService.create(payload)
  }

  private async handleRead(payload: { id?: string, page?: number, pageSize?: number }): Promise<EntityTypeMap[T] | { items: EntityTypeMap[T][], totalCount: number }> {
    return this.backendService.read(payload)
  }

  private async handleUpdate(payload: { id: string, data: Partial<EntityTypeMap[T]> }): Promise<EntityTypeMap[T]> {
    return this.backendService.update(payload.id, payload.data)
  }

  private async handlePatch(payload: { id: string, data: Partial<EntityTypeMap[T]> }): Promise<EntityTypeMap[T]> {
  ...
  private async handleQuery(payload: { sql: string, page: number, pageSize: number }): Promise<{ items: any[], totalCount: number }> {
    return this.backendService.query(payload.sql, payload.page, payload.pageSize)
  }
}