...
const BACKEND_API_URL = process.env.BACKEND_API_URL || 'https://api.example.com/v1/operations'
// APPLICATION_KEY is kept server-side and not exposed to the client
const APPLICATION_KEY = process.env.APPLICATION_KEY || 'default-application-key'
...
export type EntityType = 'countries' | 'favourites' | 'tools' | 'purposes' | 'tags' | 'toolGroups' | 'users'
...
// CRUD operations
async function getAll<T extends EntityType>(entity: T, page: number = 1, pageSize: number = 10): Promise<{ items: EntityTypeMap[T][]; totalCount: number }> {
  ...
  return {
    items: entitySchemaMap[entity].array().parse(data.items) as EntityTypeMap[T][],
    totalCount: z.number().parse(data.totalCount),
  }
}

async function getOne<T extends EntityType>(entity: T, id: string): Promise<EntityTypeMap[T] | null> {
  const response = await fetch(BACKEND_API_URL, {
    method: 'POST',
    headers: { 
      'Content-Type': 'application/json',
...
async function remove<T extends EntityType>(entity: T, id: string): Promise<boolean> {
  const response = await fetch(BACKEND_API_URL, {
    method: 'POST',
    headers: { 
      'Content-Type': 'application/json',
...
async function patch<T extends EntityType>(
  ...
  const currentUserId = getCurrentUserId()
  const patchData = Object.entries(data).map(([key, value]) => ({ key, value }))
  const response = await fetch(BACKEND_API_URL, {
    method: 'POST',
    headers: { 
      'Content-Type': 'application/json',
...
export class ApiClient<T extends EntityType> {
  ...
  async getAll(page: number = 1, pageSize: number = 10): Promise<{ items: EntityTypeMap[T][]; totalCount: number }> {
    const response = await this.fetchWithAuth(`${this.baseUrl}?page=${page}&pageSize=${pageSize}`)
    if (!response.ok) throw new Error('Failed to fetch data')
    const data = await response.json()
    return {
      items: entitySchemaMap[this.entity].array().parse(data.items) as EntityTypeMap[T][],
      totalCount: z.number().parse(data.totalCount),
    }
  }
  ...
  async create(data: Omit<EntityTypeMap[T], keyof z.infer<typeof baseEntitySchema>>): Promise<EntityTypeMap[T]> {
     //TODO: Validate data
  ...
  async update(id: string, data: Partial<Omit<EntityTypeMap[T], keyof z.infer<typeof baseEntitySchema>>>): Promise<EntityTypeMap[T]> {
     //TODO: Validate data
  ...
  async query<R>(sql: string, page: number = 1, pageSize: number = 10, mapFunction: (row: any) => R): Promise<{ items: R[]; totalCount: number }> {
  ...
  async patch(id: string, data: Partial<Omit<EntityTypeMap[T], keyof z.infer<typeof baseEntitySchema>>>): Promise<EntityTypeMap[T]> {
     //TODO: Validate data
    //const validatedData = entitySchemaMap[this.entity].partial().omit(baseEntitySchema.keyof().enum).parse(data)
    const response = await this.fetchWithAuth(`${this.baseUrl}?id=${id}`, {
      method: 'PATCH',
