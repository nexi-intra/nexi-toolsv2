...
type FileSystemActions = {
  ...
  writeFile: { payload: { path: string, content: string }, result: { success: boolean, message: string } }
  appendFile: { payload: { path: string, content: string }, result: { success: boolean, message: string } }
  deleteFile: { payload: { path: string }, result: { success: boolean, message: string } }
...
async function scanComponent(filePath: string) {
  ...
  for (const line of lines) {
    if (line.includes('export const SUGGESTED_PATH =')) {
      suggestedPath = line.split('=')[1].trim().replace(/['"`;]/g, '')
    }
    ...
    if (line.includes('export const SUGGESTED_FILE =')) {
      suggestedFile = line.split('=')[1].trim().replace(/['"`;]/g, '')
    }
    if (line.includes('export const SUGGESTED_DISPLAYNAME =')) {
      suggestedDisplayName = line.split('=')[1].trim().replace(/['"`;]/g, '')
    }
  }
...
async function generateDocumentation(componentPath: string) {
  const { suggestedPath, exampleFunc, suggestedFile, suggestedDisplayName } = await scanComponent(componentPath)
  ...
  const docPath = path.join(process.cwd(), 'app', appName, 'docs', 'components', suggestedPath)
...
export async function handleFileSystemRequest(request: NextRequest, { params }: { params: { slug: string[] } }) {
    ...
    if (request.method === 'GET') {
      switch (operation) {
        ...
        case 'fileexists':
          ...
          break
        case 'scancomponent':
          if (slugParams.length < 1) throw new Error('Component path is required')
      ...
      switch (operation) {
        case 'writefile':
          if (slugParams.length < 1) throw new Error('File path is required')
          result = await actions.writeFile({ path: slugParams[0], content: body.content })
          break
        case 'appendfile':
          if (slugParams.length < 1) throw new Error('File path is required')
          result = await actions.appendFile({ path: slugParams[0], content: body.content })
          break
        case 'deletefile':
          if (slugParams.length < 1) throw new Error('File path is required')
...
export async function generateFileSystemApiOpenApiDefinition(
  ...
  registry.register("GenerateDocumentationResponse", GenerateDocumentationResponseSchema)
...
export const fileSystem = {
  ...
  writeFile: (path: string, content: string) => actions.writeFile({ path, content }),
  appendFile: (path: string, content: string) => actions.appendFile({ path, content }),
  ...
  generateFileSystemApiOpenApiDefinition: (registry: OpenAPIRegistry) => generateFileSystemApiOpenApiDefinition(registry),
  handleFileSystemRequest: (request: NextRequest, params: { params: { slug: string[] } }) => handleFileSystemRequest(request, params),
}
